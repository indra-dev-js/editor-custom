  ...
  function (require, exports, module) {
    ...
    var AcePopup = /** @class */ (function () {
      function AcePopup(parentNode) {
        ...
        popup.renderer.setStyle('ace_autocomplete bebek');
        ...
        popup.renderer.on('afterRender', function () {
          ...
          for (
            ...
            var autocompleteIcon = dom.createElement('span');
            autocompleteIcon.className = 'ace_completion_icon ';
            autocompleteIcon.textContent = ' ';
            if (
              popupRowElement.firstChild &&
              !popupRowElement.querySelector('.ace_completion_icon')
            ) {
              popupRowElement.insertBefore(
                autocompleteIcon,
                popupRowElement.firstChild,
              );
            } else if (!popupRowElement.firstChild) {
              popupRowElement.appendChild(autocompleteIcon);
            }
            ...
            popupRowElement
              .querySelectorAll('.ace_completion_icon')
              .forEach(icon => {
                const type = rowData.type || 'unknown';
                if (type === 'function') {
                  //ace-lang-completion-class

                  icon.classList.add(
                    'ace-lang-completion-method',
                    'ace-lang-completion-function',
                  );
                } else if (type === 'object') {
                  icon.classList.add('ace-lang-completion-var');
                } else if (type === 'property') {
                  icon.classList.add('ace-lang-completion-property'); // bisa bikin icon berbeda juga
                } else if (rowData.type === 'keyword') {
                  icon.classList.add('ace-lang-completion-keyword'); // bisa bikin icon berbeda juga

                } else if (type === "class") {
                     icon.classList.add("ace-lang-completion-class"); 
                } else if (type === "constant") {
                    // ace-lang-completion-constant
                      icon.classList.add('ace-lang-completion-constant');
                }
                else {
                  icon.classList.add('ace-lang-completion-keyword');
                }
              });
        ...
        popup.tryShow = function (pos, lineHeight, anchor, forceShow) {
          ...
          if (anchor === 'top') {
            ...
            el.style.bottom =
              ...
              scrollBarSize -
              dims.bottom -
              (rootRect ? screenHeight + scrollBarSize - rootRect.bottom : 0) +
              'px';
            popup.isTopdown = false;
          } else {
  ...
  function (require, exports, module) {
    ...
    var Autocomplete = /** @class */ (function () {
      ...
      Autocomplete.prototype.$updateGhostText = function (completion) {
        ...
        var prefix = this.editor.session
          .getLine(row)
          .slice(column, cursorColumn);
      ...
      Autocomplete.prototype.showDocTooltip = function (item) {
        ...
        if (max == scores[0]) {
          ...
          tooltipStyle.maxWidth =
            Math.min(targetWidth, window.innerWidth) + 'px';
          if (popup.isTopdown) {
    ...
    var MarkerGroup = /** @class */ (function () {
      ...
      MarkerGroup.prototype.update = function (
        ...
        for (var i = 0; i < this.markers.length; i++) {
          ...
          var markerVisibleRange = marker.range.clipRows(
            visibleRangeStartRow,
            visibleRangeEndRow,
          );
          if (
            markerVisibleRange.start.row === markerVisibleRange.end.row &&
          ...
          if (screenRange.isEmpty()) {
            foldLine = session.getNextFoldLine(
              markerVisibleRange.end.row,
              foldLine,
            );
